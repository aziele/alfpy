"""Substitution matrices for use in alignments

The information on this page was originally obtained from BioPython package
...
def get(name):
    alphabet_list = get_alphabet_list(name)
    size = len(alphabet_list)
    data = np.zeros([size, size])
    for aa1, aa2 in list(d[name].keys()):
        i = alphabet_list.index(aa1)
        j = alphabet_list.index(aa2)
        data[i, j] = d[name][(aa1, aa2)]
        data[j, i] = d[name][(aa1, aa2)]
    return SubsMat(name=name, data=data, alphabet_list=alphabet_list)
...
def main():
    name = 'pam250'
    print(get_alphabet_list(name))
    m = get(name)
    print(m.alphabet_list)
    print(list_subsmats())
