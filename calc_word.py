...
def validate_args(parser):
    ...
    if args.vector == 'freqs_std':
        if args.char_freqs == None and args.alphabet_size == None:
            e = "freqs_std requires either --alphabet_size or --char_freqs"
            parser.error(e)
        elif args.char_freqs != None:
            try:
...
def main():
    ...
    if args.vector == 'counts' or args.vector == 'freqs':
        ...
        if args.alphabet_size:
            freqmodel = word_vector.EqualFreqs(alphabet_size=args.alphabet_size)
        else:
